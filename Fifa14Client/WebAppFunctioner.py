...
class WebAppFunctioner(object):
    ...
    """
    COIN_URL = 'https://utas.%sfut.ea.com/ut/game/fifa15/user/credits'
    TRANSFER_URL = 'https://utas.%sfut.ea.com/ut/game/fifa15/transfermarket?type=%s&lev=%s' \
                   '&pos=%s&num=%s&team=%s&macr=%s&micr=%s&minb=%s&maxb=%s' \
                   '&playStyle=%s&leaga=%s&start=%s&cat=%s&definitionId=%s&maskedDefId=%s'

    BID_URL = 'https://utas.%sfut.ea.com/ut/game/fifa15/trade/%s/bid'
    MOVE_URL = 'https://utas.%sfut.ea.com/ut/game/fifa15/item'
    UNASSIGNED_URL = 'https://utas.%sfut.ea.com/ut/game/fifa15/purchased/items'
    LIST_CARD_URL = 'https://utas.%sfut.ea.com/ut/game/fifa15/auctionhouse'
    TRADEPILE_URL = 'https://utas.%sfut.ea.com/ut/game/fifa15/tradepile'
    SEND_TO_TRANSFER_URL = 'https://utas.%sfut.ea.com/ut/game/fifa15/item/resource'
    QUICKSELL_URL = 'https://utas.%sfut.ea.com/ut/game/fifa15/item/%s'
    WATCHLIST_URL = 'https://utas.%sfut.ea.com/ut/game/fifa15/watchlist'
    WATCHLIST_REMOVE_URL = 'https://utas.%sfut.ea.com/ut/game/fifa15/watchlist?tradeId=%s'
    TRADEPILE_REMOVE_URL = 'https://utas.%sfut.ea.com/ut/game/fifa15/trade/%s'
    CLUB_PILES_URL = 'https://utas.%sfut.ea.com/ut/game/fifa15/club/consumables/%s'
    SQUAD_URL = 'https://utas.%sfut.ea.com/ut/game/fifa15/squad/%s'
    CLUB_URL = 'https://utas.%sfut.ea.com/ut/game/fifa15/club?count=%s&level=%s&type=%s&start=%s'
    ACCOUNT_URL = 'https://utas.%sfut.ea.com/ut/game/fifa15/user'
    LEADER_URL = 'https://utas.%sfut.ea.com/ut/game/fifa15/leaderboards/period/alltime/user/%s'
    RELIST_URL = 'https://utas.%sfut.ea.com/ut/game/fifa15/auctionhouse/relist'
    ...
    def search(self, type="", lev="gold", pos="", num=10, team="",
               macr="", micr="", minb="", maxb="",
        ...
        """
        the_url = self.TRANSFER_URL % (self.platform_string,type, lev, pos, num, team, macr, micr, minb, maxb,
                                       playStyle, leag, start, cat, definitionId, maskedDefId)
        parsed = urlparse.urlparse(the_url)
        pars = {}
        for (key, value) in urlparse.parse_qsl(parsed.query):
            if len(value) != 0:
                pars[key] = value
        #print pars
        the_url = parsed.scheme + "://" + parsed.netloc + parsed.path + "?" +  urllib.urlencode(pars)
        #print the_url
    ...
    def move(self, card, pile):
        ...
        try:
            json = r.json()
            print json
            if json["itemData"][0]["success"] == False:
                print "False 1"
                return False
        except:
    ...
    def list_card(self, card, starting_bid, buy_now_price=0, duration=3600):
        ...
        """
        payload = '{"itemData":{"id":%s}, "buyNowPrice":%s, "startingBid":%s, "duration":%s}' % \
                  (card.id, buy_now_price, starting_bid, duration)
        r = requests.post(self.LIST_CARD_URL % self.platform_string, headers=self.get_headers('POST'), data=payload)
        self.rest()
        #print self.LIST_CARD_URL % self.platform_string, payload
    ...
    def relist_all(self):
        """Relists all cards!"""
        r = requests.post(self.RELIST_URL % self.platform_string, headers=self.get_headers('PUT'))
        self.rest()
        try:
            json = r.json()
            print json
    ...
    def get_club_pile(self, pile):
        """Returns a list of Card objects from the training pile in consumabled, raises an exception on failure."""
        r = requests.post(self.CLUB_PILES_URL % (self.platform_string, pile), headers=self.get_headers('GET'))
        self.rest()
        try:
            json = r.json()
        except:
        ...
        if 'itemData' in json:
            card_list = json['itemData']
            #print card_list
            return [card_dict for card_dict in card_list]
        elif 'code' in json:
            raise FUTErrorCodeException("Could not get training pile.",json)

    def move_to_transfer_list(self, resourceId):
        """Moves card to specified pile, raises an exception on failure"""
        payload = '{"itemData":[{"id":"%s","pile":"trade"}]}' % (resourceId)
        r = requests.post(self.SEND_TO_TRANSFER_URL % self.platform_string, headers=self.get_headers('PUT'), data=payload)
        self.rest()
        #There will be no code key in the json if the move is sucessful
        try:
            json = r.json()
